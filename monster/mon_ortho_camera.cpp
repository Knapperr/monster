...
namespace Mon
{
	...
	void InitCamera(Camera2D* camera, Rect viewPort)
	{
		...
		camera->resolution.w = viewPort.w / 4.0f;
		camera->resolution.h = viewPort.h / 4.0f;
	...
	mat4 ViewMatrix(Camera2D* camera)
	{
		...
		float pixelsToMeters = 1.0f / 16.0f; // 1m / 16 pixels = 0.0625f?
		//v3 cameraPosToPixelPos = v3(camera->pos, 0.0f);
		...
		//view = glm::translate(view, v3(-(cameraPosToPixelPos.x), -(cameraPosToPixelPos.y), 1.0f));
